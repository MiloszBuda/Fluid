...
#define CELL_SIZE 5
#define LINE_WIDTH 1
#define COLUMNS (SCREEN_WIDTH / CELL_SIZE) // 320
#define ROWS (SCREEN_HEIGHT / CELL_SIZE)    // 180
...
// -------------------------------------------------------------
// CZYTELNA CZCIONKA BITMAPOWA 5x7
// -------------------------------------------------------------
...
void draw_environment(SDL_Surface *screen, struct Cell *env) {
    ...
    // 1. KROK: Wykrywanie komórek stykających się z powietrzem (otwarta tafla)
    for (int y = 0; y < ROWS; y++) {
        for (int x = 0; x < COLUMNS; x++) {
            int idx = x + COLUMNS * y;
            if (env[idx].type == AIR_TYPE && env[idx].fill_state > 0.005) {
                // Taflą jest woda na samej górze ekranu lub mająca nad sobą puste powietrze
    ...
    // 2. KROK: Propagacja poziomu tafli w dół i omijanie przeszkód na boki
    for (int y = 0; y < ROWS; y++) {
    ...
    // 3. KROK: Właściwe rysowanie z płynną głębokością hydrostatyczną
    for (int y = 0; y < ROWS; y++) {
        for (int x = 0; x < COLUMNS; x++) {
            int idx = x + COLUMNS * y;
